# Kickoff — Memory + Ledger

Paste this to the agent (working at the repo root):

---

You are the **Memory + Ledger** subteam for FunTog, an agentic developer working in this repository.
Your workspace is `services/memory-ledger/` and you work ONLY there.

Before writing any code, read, in order:
1. `services/memory-ledger/AGENTS.md` — your full brief: mission, the **defining decision you must not
...
2. `services/memory-ledger/docs/` — the subsystem deep-dive (if present).
3. `docs/architecture/funtog-architecture.md` — §7 is the contract matrix.
4. `AGENTS.md` (repo root) — rules of engagement.

Your task:
- Implement `MemoryService` from `@funtog/contracts` by replacing the stub `createMemory()` in
  `services/memory-ledger/src/index.ts` with a real implementation.
...
Definition of done:
- `npm test -w services/memory-ledger` is GREEN — this runs the shared contract suite
  (`services/memory-ledger/tests/contract.test.ts`) against your implementation.

# Kickoff — Venue / Knowledge

Paste this to the agent (working at the repo root):

---

You are the **Venue / Knowledge** subteam for FunTog, an agentic developer working in this repository.
Your workspace is `services/venue-knowledge/` and you work ONLY there.

Before writing any code, read, in order:
1. `services/venue-knowledge/AGENTS.md` — your full brief: mission, the **defining decision you must not
...
2. `services/venue-knowledge/docs/` — the subsystem deep-dive (if present).
3. `docs/architecture/funtog-architecture.md` — §7 is the contract matrix.
4. `AGENTS.md` (repo root) — rules of engagement.

Your task:
- Implement `VenueService` from `@funtog/contracts` by replacing the stub `createVenue()` in
  `services/venue-knowledge/src/index.ts` with a real implementation.
...
Definition of done:
- `npm test -w services/venue-knowledge` is GREEN — this runs the shared contract suite
  (`services/venue-knowledge/tests/contract.test.ts`) against your implementation.

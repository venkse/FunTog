# Kickoff — Crew + Identity

Paste this to the agent (working at the repo root):

---

You are the **Crew + Identity** subteam for FunTog, an agentic developer working in this repository.
Your workspace is `services/crew-identity/` and you work ONLY there.

Before writing any code, read, in order:
1. `services/crew-identity/AGENTS.md` — your full brief: mission, the **defining decision you must not
...
2. `services/crew-identity/docs/` — the subsystem deep-dive (if present).
3. `docs/architecture/funtog-architecture.md` — §7 is the contract matrix.
4. `AGENTS.md` (repo root) — rules of engagement.

Your task:
- Implement `CrewService` from `@funtog/contracts` by replacing the stub `createCrew()` in
  `services/crew-identity/src/index.ts` with a real implementation.
...
Definition of done:
- `npm test -w services/crew-identity` is GREEN — this runs the shared contract suite
  (`services/crew-identity/tests/contract.test.ts`) against your implementation.

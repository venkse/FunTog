# Kickoff — Suggestion Engine

Paste this to the agent (working at the repo root):

---

You are the **Suggestion Engine** subteam for FunTog, an agentic developer working in this repository.
Your workspace is `services/suggestion-engine/` and you work ONLY there.

Before writing any code, read, in order:
1. `services/suggestion-engine/AGENTS.md` — your full brief: mission, the **defining decision you must not
...
2. `services/suggestion-engine/docs/` — the subsystem deep-dive (if present).
3. `docs/architecture/funtog-architecture.md` — §7 is the contract matrix.
4. `AGENTS.md` (repo root) — rules of engagement.

Your task:
- Implement `SuggestionService` from `@funtog/contracts` by replacing the stub `createSuggestion()` in
  `services/suggestion-engine/src/index.ts` with a real implementation.
...
Definition of done:
- `npm test -w services/suggestion-engine` is GREEN — this runs the shared contract suite
  (`services/suggestion-engine/tests/contract.test.ts`) against your implementation.

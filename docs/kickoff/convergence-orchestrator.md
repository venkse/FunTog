You are the subteam building the **Convergence Orchestrator (sessions, votes, deterministic verdict, wheel authority)** for FunTog. An agentic build is running in parallel;
discipline is what keeps it working.

Before writing code, read, in order:
1. `AGENTS.md` (repo root) — the rules of engagement.
2. `services/convergence-orchestrator/AGENTS.md` — your brief: mission, defining decision, contracts, components,
   degradation, definition of done.
3. `services/convergence-orchestrator/docs/` — your subsystem deep-dive (if present).
4. `docs/architecture/funtog-architecture.md` §7 — the contract matrix.

Hard rules:
- Edit ONLY `services/convergence-orchestrator/`. Never edit another service or `packages/contracts`.
...
- In `services/convergence-orchestrator/tests/contract.test.ts`, replace the mock with YOUR implementation.

Definition of done: `npx vitest run services/convergence-orchestrator` is green with your real implementation wired

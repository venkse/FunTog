You are the subteam building the **Crew + Identity (crews, link-join, lightweight identity)** for FunTog. An agentic build is running in parallel;
discipline is what keeps it working.

Before writing code, read, in order:
1. `AGENTS.md` (repo root) — the rules of engagement.
2. `services/crew-identity/AGENTS.md` — your brief: mission, defining decision, contracts, components,
   degradation, definition of done.
3. `services/crew-identity/docs/` — your subsystem deep-dive (if present).
4. `docs/architecture/funtog-architecture.md` §7 — the contract matrix.

Hard rules:
- Edit ONLY `services/crew-identity/`. Never edit another service or `packages/contracts`.
...
- In `services/crew-identity/tests/contract.test.ts`, replace the mock with YOUR implementation.

Definition of done: `npx vitest run services/crew-identity` is green with your real implementation wired
